...
export default function FormTrial({ selected, onClose, bookingDetails }) {
  ...
  const handleSubmit = (event) => {
    ...
    // Prepare the info object for the booking
    const info = {
      level: teachingLevel,
      from: selectedCountry,
      motherTongue: choosenLanguages.join(", "), // Assuming multiple languages are joined by comma
    };
    console.log(info);
    // Dispatch the createBooking action
    dispatch(
      acceptBooking({
        bookingId: bookingDetails.id, // Ensure this matches your actual booking ID
        languageId: choosenLanguages[0].id, // Adjust as needed
        info,
      })
    )
      .unwrap()
